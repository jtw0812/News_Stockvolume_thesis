...
stopwords = {
    ...
    "쾅쾅", "쿵", "타다", "타인", "탕탕", "토하다", "통하여", "툭", "퉤", "틈타", "팍", "팔", "퍽", "펄렁", "하", "하게될것이다",
    ...
    "않", "없", "받", "보", "위하", "통하", "els", "수수료", "배포", "가장", "종합", "보", "기록", "종목", "늘", "크", "줄", "가장", "침", "같", "많", "열", "사업", "산업",
    "기초자산", "장", "파트", "너스" , "미" , "면" , "침" , "높", "무단", "전재", '우스', '대해',
    "청춘", "합쳐", "정", "안", "수익률", "펀드", "상장", "배당", "시장", "흐름", "촬영", "더", "사람", "좋",
    "조치", "입장", "결정", "부", "구독", "채널", "이슈", "전", "생각",  "마감", "etf", "고객", "업계", "다양", "진행", "제공", "예정", "가능성", "대비", "쇼", "타", "상품", "지급", "공모", "미만", "곳", "기준", "적용", "빡", "만나", "미리","플", "맺",
    "이노비즈", "증하", "친족", "하계", "포켓몬고", "토리", "레몬", "즐겁", "종교", "스텐", "인차", "서가", "한데다", "베브", "이노비즈", "살롱", "아이디", "구구", "문복", "후원자", "곰", "시알", "트라이얼", "계란", "심비", "팡팡", "라니티딘", "세인", "서도", "놀이터", "개띠", "누스", "말벌", "남편", "치아", "술년", "선다소견", "불탑", "살펴보", "간추린", "서", "각국", "첫해", "앞장서", "숙박", "진의", "유령", "우와", "학교", "획득", "어둡", "다행히", "제미", "당첨", "맞춤", "디", "매일", "품목", "합니다", "쿠폰", "패치", "됩니다", "인원", "재테크", "안방", "시세", "무료", "실시간", "발행", "상담센터", "한달", "경품", "상담", "해선", "왜건", "둥", "허브", "취","세타", "멤버스", "매매", "드라이빙", "상범","튀","넉넉", "세미나","시절","안내","모양","말씀", "비보장형","밴","ecb","컨퍼런스","가득","저작권자","소통","매일","양궁","한눈","예술","배우","민관","무분규","스타","우승","드려요","화보",
    "친구", "나주", "소속", "직원", "국민", "연봉", "자료", "서서히", "애플힙", "주자", "문자", "진단", "기사", "추천", "만기", "월드컵", "캠프", "선착순", "전형", "사망", "문의", "잼", "꿀", "선수","퀴즈","마을", "인간", "월드","박람회","눈","양국","베","엑스포","오피니언","용익","출전","귀국","시티","무대","색상","커뮤니케이션","출",
    "오피스텔", "판", "이벤트", "씽", "공짜", "팁", "드린다", "송심", "오희", "우주", "기흥", "핏", "잇","큐", "서초", "즉각", "일시", "서류", "딸", "편하", "뮤지컬", "하만", "비트", 
}
...
def remove_stopwords(list):
    filtered_tokens = [token for token in list if token not in stopwords]
    return filtered_tokens

def get_tokens(text):
    tokens = mpck.tokenize(text)
    token = [word.split('/')[0] for word in tokens]
    result = remove_stopwords(token)
    return result

def column_tokenize(df, column):
    df['tokens'] = df[column].apply(lambda text: get_tokens(text))
    return df
